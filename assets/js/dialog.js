...
(function(window) {
    ...
    MoBookingDialog.prototype._create = function() {
        ...
        this.overlayEl.className = 'mobooking-dialog-overlay';

        // Create dialog
        this.dialogEl = document.createElement('div');
        this.dialogEl.className = 'mobooking-dialog';
        this.dialogEl.setAttribute('role', 'dialog');
        this.dialogEl.setAttribute('aria-modal', 'true');
        this.dialogEl.setAttribute('aria-labelledby', 'mobooking-dialog-title');

        // Header
        const header = document.createElement('div');
        header.className = 'mobooking-dialog-header';

        let iconHtml = '';
        if (this.options.icon) {
            const iconSvg = ICONS[this.options.icon] || this.options.icon;
            iconHtml = `<div class="mobooking-dialog-icon">${iconSvg}</div>`;
        }

        header.innerHTML = `
            ${iconHtml}
            <h2 id="mobooking-dialog-title" class="mobooking-dialog-title">${this.options.title}</h2>
            <button class="mobooking-dialog-close-btn">
                ${ICONS.close}
            </button>
        `;
        ...
        content.className = 'mobooking-dialog-content';
        ...
        if (this.options.buttons.length > 0) {
            ...
            footer.className = 'mobooking-dialog-footer';
        ...
        this.dialogEl.querySelector('.mobooking-dialog-close-btn').addEventListener('click', () => this.close());
    ...
    MoBookingDialog.prototype.close = function() {
        ...
        this.dialogEl.addEventListener('animationend', handleAnimationEnd, { once: true });
    };
