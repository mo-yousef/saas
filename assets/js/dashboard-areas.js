jQuery(document).ready(function($) {
    ...
    function renderTemplate(templateId, data) {
        ...
        for (const key in data) {
            ...
            currentItemHtml = currentItemHtml.replace(new RegExp('<%=\\s*' + key + '\\s*%>', 'g'), sanitizeHTML(String(value)));
        }
    ...
    function fetchAndRenderAreas(page = 1) {
        ...
        areasListContainer.html('<p>' + (mobooking_areas_params.i18n.loading || 'Loading...') + '</p>');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success && response.data.areas && response.data.areas.length) {
                    ...
                    areasListContainer.html('<p>' + (mobooking_areas_params.i18n.no_areas || 'No areas found.') + '</p>');
                } else {
                    areasListContainer.html('<p>' + sanitizeHTML(response.data.message || mobooking_areas_params.i18n.error_loading) + '</p>');
                }
            },
            error: function() {
                areasListContainer.html('<p>' + (mobooking_areas_params.i18n.error_loading || 'Error loading areas.') + '</p>');
            }
        });
    }
    ...
    function resetForm() {
        areaForm[0].reset();
        areaIdField.val('');
        areaFormTitle.text(mobooking_areas_params.i18n.add_title || 'Add New Service Area');
    ...
    areaForm.on('submit', function(e) {
        ...
        if (!countryCode || !areaValue) {
            feedbackDiv.text(mobooking_areas_params.i18n.fields_required || 'Country code and ZIP/Area value are required.').addClass('error').show();
            return;
        }

        const originalButtonText = saveAreaBtn.text();
        saveAreaBtn.prop('disabled', true).text(mobooking_areas_params.i18n.saving || 'Saving...');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    feedbackDiv.text(response.data.message || mobooking_areas_params.i18n.error_general).removeClass('success').addClass('error').show();
                }
            },
            error: function() {
                feedbackDiv.text(mobooking_areas_params.i18n.error_general || 'An AJAX error occurred.').removeClass('success').addClass('error').show();
            },
            complete: function() {
    ...
    areasListContainer.on('click', '.mobooking-edit-area-btn', function() {
        ...
        areaFormTitle.text(mobooking_areas_params.i18n.edit_title || 'Edit Service Area');
        saveAreaBtn.text(mobooking_areas_params.i18n.save_button || 'Save Changes');
        cancelEditBtn.show();
        feedbackDiv.empty().hide();
        $('html, body').animate({ scrollTop: areaFormWrapper.offset().top - 50 }, 500); // Scroll to form
    });
    ...
    areasListContainer.on('click', '.mobooking-delete-area-btn', function() {
        ...
        if (confirm(mobooking_areas_params.i18n.confirm_delete || 'Are you sure you want to delete this area?')) {
            $.ajax({
                url: mobooking_areas_params.ajax_url,
                type: 'POST',
                data: {
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        alert(response.data.message || mobooking_areas_params.i18n.deleted_successfully);
                    } else {
                        alert(sanitizeHTML(response.data.message || mobooking_areas_params.i18n.error_deleting));
                    }
                },
                error: function() {
                    alert(mobooking_areas_params.i18n.error_deleting || 'AJAX error deleting area.');
                }
            });
        }
    });
    ...
    if (typeof mobooking_areas_params === 'undefined') {
        console.error('mobooking_areas_params is not defined. Please ensure it is localized correctly.');
