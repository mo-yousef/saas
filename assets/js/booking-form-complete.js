...
function getNextActiveStep(currentStep) {
    ...
    return currentIndex < activeSteps.length - 1 ? activeSteps[currentIndex + 1] : null;
}
...
function showStep(stepNumber) {
    ...
    if (!stepConfig[stepNumber] || !stepConfig[stepNumber].enabled) {
        console.log("[MoBooking] Step", stepNumber, "is disabled, finding next active step");
...
function updateProgressBar() {
    ...
    $("#mobooking-progress-text").text("Step " + currentStepIndex + " of " + totalSteps);
}
...
function updateFrequencyDisplay(frequency) {
    ...
    $('input[name="service_frequency"][value="' + frequency + '"]').closest('.mobooking-frequency-card').addClass('selected');
}
...
function loadAvailableTimeSlots(date) {
    ...
    // Show loading state
    $('#available-time-slots').html('<div class="mobooking-loading">Loading available times...</div>').show();
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success && response.data.time_slots) {
                ...
                $('#available-time-slots').html('<p>No available times for this date.</p>');
            }
        },
        error: function() {
...
function submitBooking() {
    ...
    $submitBtn.prop('disabled', true).html('<div class="mobooking-spinner"></div> Submitting...');
    ...
    const bookingData = {
        ...
        customer_details: JSON.stringify({
            ...
            address: {
                ...
                state: enhancedFormData.customer.state || enhancedFormData.location.state,
                zip_code: enhancedFormData.customer.zipCode || enhancedFormData.location.zipCode
            },
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success) {
                ...
                showFeedback('error', response.data.message || 'Booking submission failed. Please try again.');
                $submitBtn.prop('disabled', false).html(originalBtnText);
            }
        },
        error: function(xhr, status, error) {
            console.error("[MoBooking] Booking submission error:", error);
            showFeedback('error', 'An error occurred while submitting your booking. Please try again.');
            $submitBtn.prop('disabled', false).html(originalBtnText);
        }
    });
}
...
function showBookingConfirmation(confirmationData) {
    ...
    detailsHTML +=
        <div class="mobooking-confirmation-item">
            <strong>Service Date:</strong> ${formatDate(enhancedFormData.dateTime.date)} at ${formatTime(enhancedFormData.dateTime.time)}
        </div>
        <div class="mobooking-confirmation-item">
            <strong>Service Address:</strong> ${enhancedFormData.customer.streetAddress}
            ${enhancedFormData.customer.apartment ? ', ' + enhancedFormData.customer.apartment : ''}
        </div>
        <div class="mobooking-confirmation-item">
            <strong>Service Frequency:</strong> ${enhancedFormData.serviceFrequency}
        </div>
    ;
    ...
    detailsHTML +=
        <div class="mobooking-confirmation-item">
            <strong>Property Access:</strong> ${getPropertyAccessLabel(enhancedFormData.propertyAccess.method)}
            ${enhancedFormData.propertyAccess.details ? '<br><em>' + enhancedFormData.propertyAccess.details + '</em>' : ''}
        </div>
    ;
    ...
    if (confirmationData.total_amount) {
        detailsHTML +=
            <div class="mobooking-confirmation-item">
                <strong>Total Amount:</strong> ${formatCurrency(confirmationData.total_amount)}
            </div>
        ;
    }
