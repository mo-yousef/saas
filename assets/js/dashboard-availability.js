jQuery(document).ready(function ($) {
    ...
    daysOfWeek.forEach((dayName, dayIndex) => {
      ...
      const $dayItem = $(`
        <li class="day-schedule ${
          dayData.is_enabled ? "day-enabled" : "day-disabled"
        }">
      ...
      if (dayData.is_enabled) {
        if (dayData.slots.length > 0) {
          ...
          dayData.slots.forEach((slot, slotIndex) => {
            $slotsContainer.append(
              renderSlotInput(dayIndex, slotIndex, slot, totalSlots)
            );
          });
        } else {
        ...
        $slotsContainer.html(
          `<p class="day-off-text">${i18n.day_off_text || "Unavailable"}</p>`
        );
      }
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          showFeedback(
            i18n.error_loading_schedule || "Error loading schedule.",
            "error"
          );
        }
      },
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          showFloatingAlert(
            response.data.message || "Error saving schedule.",
            "error"
          );
        }
      },
  ...
  $scheduleContainer.on("change", ".day-toggle-switch", function () {
    ...
    $dayItem
      .toggleClass("day-enabled", isEnabled)
      .toggleClass("day-disabled", !isEnabled);
    ...
    if (isEnabled) {
      ...
      dayData.slots.forEach((slot, index) => {
        $slotsContainer.append(
          renderSlotInput(dayIndex, index, slot, totalSlots)
        );
      });
    } else {
      $slotsContainer.html(
  ...
  $scheduleContainer.on("click", ".add-slot-btn", function () {
    ...
    const newEndTime =
      ("0" + newStartDate.getHours()).slice(-2) +
      ":" +
      ("0" + newStartDate.getMinutes()).slice(-2);
  ...
  $scheduleContainer.on("click", ".delete-slot-btn", function () {
    ...
    if (totalSlots > 0) {
      dayData.slots.forEach((slot, newSlotIndex) => {
        $slotsContainer.append(
          renderSlotInput(dayIndex, newSlotIndex, slot, totalSlots)
        );
      });
    } else {
  ...
  $scheduleContainer.on("click", ".copy-schedule-btn", function () {
    ...
    const sourceDayData = scheduleData.find(
