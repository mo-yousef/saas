jQuery(document).ready(function ($) {
            ...
            <div class="NORDBOOKING-table-responsive-wrapper">
                <table class="NORDBOOKING-table">
                    <thead>
                        <tr>
                            <th>${
                              nordbooking_bookings_params.i18n.ref || "Ref"
                            }</th>
                            <th>${
                              nordbooking_bookings_params.i18n.customer ||
                              "Customer"
                            }</th>
                            <th>${
                              nordbooking_bookings_params.i18n.booked_date ||
                              "Booked Date"
                            }</th>
                            <th>${
                              nordbooking_bookings_params.i18n.assigned_staff ||
                              "Assigned Staff"
                            }</th>
                            <th>${
                              nordbooking_bookings_params.i18n.total || "Total"
                            }</th>
                            <th>${
                              nordbooking_bookings_params.i18n.status ||
                              "Status"
                            }</th>
                            <th>${
                              nordbooking_bookings_params.i18n.actions ||
                              "Actions"
                            }</th>
                        </tr>
                    </thead>
  ...
  function loadBookings(page = 1) {
    ...
    bookingsListContainer.html('<div class="NORDBOOKING-spinner"></div>'); // Show spinner
    paginationContainer.empty();

    let ajaxData = {
      action: "nordbooking_get_tenant_bookings",
      nonce: nordbooking_bookings_params.nonce,
      ...currentFilters,
    };
    ...
    const isFilterActive =
      ...
      currentFilters.assigned_staff_id_filter;
    $("#NORDBOOKING-clear-filters-btn").toggle(!!isFilterActive);

    $.ajax({
      url: nordbooking_bookings_params.ajax_url,
      type: "POST",
      data: ajaxData,
      success: function (response) {
        ...
        if (
          ...
          response.data.bookings.forEach(function (booking) {
            ...
            bookingDataForTemplate.total_price_formatted =
              currencyCode + " " + parseFloat(booking.total_price).toFixed(2);
            bookingDataForTemplate.status_display =
              nordbooking_bookings_params.statuses[booking.status] ||
              booking.status;
            ...
            try {
              ...
              bookingDataForTemplate.booking_date_formatted =
                bookingDate.toLocaleDateString();
              bookingDataForTemplate.booking_time_formatted =
            ...
            bookingDataForTemplate.icon_html =
              nordbooking_bookings_params.icons[booking.status] || "";

            bookingDataForTemplate.assigned_staff_name =
              booking.assigned_staff_name ||
              nordbooking_bookings_params.i18n.unassigned ||
              "Unassigned";

            bookingDataForTemplate.delete_button_html = "";
            if (
              nordbooking_bookings_params &&
              nordbooking_bookings_params.currentUserCanDeleteBookings
            ) {
              bookingDataForTemplate.delete_button_html = `<button class="btn btn-destructive btn-sm NORDBOOKING-delete-booking-btn" data-booking-id="${
                booking.booking_id
              }">${
                nordbooking_bookings_params.i18n.delete_btn_text || "Delete"
              }</button>`;
            }

            tableBody.append(
              renderTemplate(bookingItemTemplate, bookingDataForTemplate)
            );
          });
          renderPagination(
          ...
          const noResultsHTML = `
                        <div class="NORDBOOKING-no-results-message">
                            ...
                            <h4>${
                              nordbooking_bookings_params.i18n
          ...
          bookingsListContainer.html(
            "<p>" +
              (response.data.message ||
                nordbooking_bookings_params.i18n.error_loading_bookings ||
  ...
  function applyFilters() {
    currentFilters.status_filter = $("#NORDBOOKING-status-filter").val();
    currentFilters.date_from_filter = $("#NORDBOOKING-date-from-filter").val();
    currentFilters.date_to_filter = $("#NORDBOOKING-date-to-filter").val();
    currentFilters.search_query = $("#NORDBOOKING-search-query").val();
    currentFilters.assigned_staff_id_filter = $(
      "#NORDBOOKING-staff-filter"
    ).val();
    loadBookings(1);
  }
  ...
  $("#NORDBOOKING-search-query").on("keyup", debouncedApplyFilters);
  $("#NORDBOOKING-status-filter, #NORDBOOKING-staff-filter").on(
    "change",
    applyFilters
  );
  $(".NORDBOOKING-datepicker").on("change", applyFilters);
  ...
  // Clear filters
  $("#NORDBOOKING-clear-filters-btn").on("click", function () {
    filterForm[0].reset();
    currentFilters.status_filter = "";
    currentFilters.date_from_filter = "";
    currentFilters.date_to_filter = "";
  ...
  if (typeof $.fn.datepicker === "function") {
    ...
    $(".NORDBOOKING-datepicker").attr("type", "date"); // Fallback to native
  }
  ...
  // bookingsListContainer.on('click', '.NORDBOOKING-view-booking-details-btn', ...) - this is now a link
  ...
  $("#NORDBOOKING-add-booking-btn").on("click", function () {
    ...
    alert(
      "Add New Booking functionality to be implemented. This will require a new form/modal or a separate page."
    );
  });

  // DELETE BOOKING
  bookingsListContainer.on(
    "click",
    ".NORDBOOKING-delete-booking-btn",
    function (e) {
      e.preventDefault();
          ...
          },
          {
            ...
            onClick: (dialog) => {
              $.ajax({
                ...
                type: "POST",
                data: {
                  action: "nordbooking_delete_dashboard_booking",
                  nonce: nordbooking_bookings_params.nonce,
                ...
                success: function (response) {
                  if (response.success) {
                    ...
                    if (window.showAlert) {
                      window.showAlert(
                        response.data.message || "Booking deleted.",
                        "success"
                      );
                    }
                  } else {
