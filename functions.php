...
function nordbooking_check_subscription_status() {
    // Only run this check for business owners in the admin area.
    if ( is_admin() && current_user_can( 'nordbooking_business_owner' ) && ! current_user_can( 'manage_options' ) ) {
        $screen = get_current_screen();
        $user_id = get_current_user_id();

        // Don't lock the user out of the subscription page.
        if ( $screen && $screen->id === 'toplevel_page_nordbooking-subscription' ) {
            return;
        }
        ...
        if ( $trial_ends_at && time() < $trial_ends_at ) {
            return;
        }

        // Check for a grace period.
        if ( $subscription_status === 'past_due' ) {
            $subscription_updated_at = get_user_meta( $user_id, '_nordbooking_subscription_updated_at', true );
            if ( $subscription_updated_at && time() < strtotime( '+2 days', $subscription_updated_at ) ) {
                return;
            }
        }

        // If none of the above are true, redirect to the subscription page.
        wp_redirect( admin_url( 'admin.php?page=nordbooking-subscription' ) );
        exit;
    }
}
add_action( 'admin_init', 'nordbooking_check_subscription_status' );

function nordbooking_enqueue_subscription_assets( $hook ) {
    $screen = get_current_screen();
    if ( $screen && $screen->id === 'toplevel_page_nordbooking-subscription' ) {
        wp_enqueue_script(
            'nordbooking-subscription',
            get_template_directory_uri() . '/assets/js/dashboard-subscription.js',
            ['jquery'],
            NORDBOOKING_VERSION,
            true
        );
        wp_localize_script(
            'nordbooking-subscription',
            'nordbooking_dashboard_vars',
            [
                'nonce' => wp_create_nonce( 'nordbooking_subscription_nonce' ),
                'ajax_url' => admin_url( 'admin-ajax.php' ),
            ]
        );
    }
}
add_action( 'admin_enqueue_scripts', 'nordbooking_enqueue_subscription_assets' );
?>