...
class Bookings {
    ...
    public function handle_enhanced_booking_submission() {
        // Verify nonce
        if (!wp_verify_nonce($_POST['nonce'], 'mobooking_booking_nonce')) {
        ...
        if (is_wp_error($booking_result)) {
            ...
            wp_send_json_error(['message' => $booking_result->get_error_message()], 500);
            return;
        }
    ...
    private function validate_enhanced_booking_data($data) {
        $customer = $data['customer_details'];
        $services = $data['selected_services'];
        $pet_info = $data['pet_information'];
        $frequency = $data['service_frequency'];
        $access = $data['property_access'];

        // Validate customer details
        $required_customer_fields = ['name', 'email', 'phone'];
        foreach ($required_customer_fields as $field) {
            if (empty($customer[$field])) {
                return new WP_Error('missing_customer_field', sprintf(__('Customer %s is required.', 'mobooking'), $field));
            }
        }

        // Validate email
        if (!is_email($customer['email'])) {
            return new WP_Error('invalid_email', __('Please provide a valid email address.', 'mobooking'));
        }

        // Validate address if provided
        if (isset($customer['address']) && is_array($customer['address'])) {
            if (empty($customer['address']['street'])) {
                return new WP_Error('missing_address', __('Street address is required.', 'mobooking'));
            }
        }
        ...
        if (empty($customer['date']) || empty($customer['time'])) {
            return new WP_Error('missing_datetime', __('Service date and time are required.', 'mobooking'));
        }

        // Validate date format and ensure it's in the future
        $service_date = DateTime::createFromFormat('Y-m-d', $customer['date']);
        if (!$service_date) {
            return new WP_Error('invalid_date', __('Invalid service date format.', 'mobooking'));
        }

        if ($service_date < new DateTime('today')) {
            return new WP_Error('past_date', __('Service date cannot be in the past.', 'mobooking'));
        }

        // Validate pet information if pets are indicated
        if (isset($pet_info['has_pets']) && $pet_info['has_pets'] && empty($pet_info['details'])) {
            return new WP_Error('missing_pet_details', __('Please provide details about your pets.', 'mobooking'));
        }

        // Validate service frequency
        $valid_frequencies = ['one-time', 'weekly', 'monthly'];
        if (!in_array($frequency, $valid_frequencies)) {
            return new WP_Error('invalid_frequency', __('Invalid service frequency.', 'mobooking'));
        }

        // Validate property access
        if (isset($access['method']) && $access['method'] === 'other' && empty($access['details'])) {
            return new WP_Error('missing_access_details', __('Please provide property access details.', 'mobooking'));
        }
    ...
    private function create_enhanced_booking($booking_data) {
        ...
        $insert_data = [
            ...
            'customer_name' => $booking_data['customer_details']['name'],
            'customer_email' => $booking_data['customer_details']['email'],
            'customer_phone' => $booking_data['customer_details']['phone'],
            'service_address' => $this->format_service_address($booking_data['customer_details']['address'] ?? []),
            'service_date' => $booking_data['service_date'],
            'service_time' => $booking_data['service_time'],
            'selected_services' => wp_json_encode($booking_data['selected_services']),
            'service_options' => wp_json_encode($booking_data['service_options']),
            'service_frequency' => $booking_data['service_frequency'],
            'has_pets' => $booking_data['pet_information']['has_pets'] ? 1 : 0,
            'pet_details' => $booking_data['pet_information']['details'] ?? '',
            'property_access_method' => $booking_data['property_access']['method'] ?? 'home',
            'property_access_details' => $booking_data['property_access']['details'] ?? '',
            'special_instructions' => $booking_data['customer_details']['instructions'] ?? '',
            'booking_status' => $booking_data['booking_status'],
            'total_amount' => $booking_data['pricing_data']['final_total'] ?? 0,
            'booking_metadata' => wp_json_encode([
                'location_data' => $booking_data['location_data'],
                'pricing_breakdown' => $booking_data['pricing_data'],
                'submission_timestamp' => current_time('timestamp'),
        ...
        if ($result === false) {
            return new WP_Error('db_insert_failed', __('Failed to create booking. Please try again.', 'mobooking'));
        }
    ...
    private function format_service_address($address_data) {
        ...
        if (!empty($address_data['apartment'])) {
            $parts[] = $address_data['apartment'];
        }
        ...
        if (!empty($address_data['zip_code'])) {
            $parts[] = $address_data['zip_code'];
        }

        return implode(', ', $parts);
    }

    private function send_enhanced_booking_confirmations($booking_id, $booking_data) {
        // This would send enhanced email confirmations
        // including all the new information (pets, frequency, access method, etc.)

        // Implementation would extend the existing email functionality
        // to include the new fields in the email templates
    }
    ...
    private function generate_booking_reference() {
        return 'MB-' . strtoupper(wp_generate_password(8, false));
    }
