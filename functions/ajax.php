...
add_action('wp_ajax_nordbooking_create_business_owner_checkout_session', 'nordbooking_create_business_owner_checkout_session');
function nordbooking_create_business_owner_checkout_session() {
    if (!check_ajax_referer('nordbooking_subscription_nonce', '_wpnonce', false)) {
    ...
    try {
        ...
        $checkout_session = $stripe_manager->create_checkout_session($user_id, $user->user_email);
...
add_action('wp_ajax_nordbooking_create_customer_portal_session', 'nordbooking_create_customer_portal_session');
function nordbooking_create_customer_portal_session() {
    if (!check_ajax_referer('nordbooking_subscription_nonce', '_wpnonce', false)) {
        wp_send_json_error(['message' => 'Nonce verification failed.'], 403);
        return;
    }
    ...
    $stripe_customer_id = get_user_meta($user_id, '_nordbooking_stripe_customer_id', true);
    if (empty($stripe_customer_id)) {
        wp_send_json_error(['message' => 'No Stripe customer ID found.'], 400);
        return;
    }

    try {
        \Stripe\Stripe::setApiKey(get_option('nordbooking_stripe_secret_key'));
        $portal_session = \Stripe\BillingPortal\Session::create([
            'customer' => $stripe_customer_id,
            'return_url' => home_url('/dashboard/subscription/'),
        ]);

        wp_send_json_success(['url' => $portal_session->url]);
