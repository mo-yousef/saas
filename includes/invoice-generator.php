 ...
 *
 * This file is responsible for generating an HTML invoice for a given booking.
...
// Security checks are implicitly handled by the calling script (page-booking-single.php)
...
// Debug: Log that we're generating an invoice
error_log('Invoice Generator: Starting invoice generation for booking ID: ' . ($single_booking_id ?? 'unknown'));

// Clean any existing output buffers to prevent dashboard content from showing
if (ob_get_level()) {
    ob_end_clean();
}

// Instantiate the Settings class and fetch business settings with error handling
try {
    ...
    // Fallback to default business settings if there's an error
...
// Prepare formatted data with fallbacks
...
// Ensure currency symbol is available
if (!isset($currency_symbol) || empty($currency_symbol)) {
    $currency_symbol = '$'; // Default fallback
}

// Check if user wants to download as HTML file or view in browser
$download_as_file = isset($_GET['download_as_file']) && $_GET['download_as_file'] === 'true';

// Set appropriate headers only if no output has been sent
if (!headers_sent()) {
    if ($download_as_file) {
...
<head>
    ...
    <style>
        /* Reset and hide all WordPress/dashboard elements */
        * {
            margin: 0;
            padding: 0;
        ...
        /* Hide all WordPress dashboard elements */
        #wpadminbar,
        .nordbooking-dashboard-header,
        .NORDBOOKING-dashboard-header,
        .wp-toolbar,
        .admin-bar,
        .dashboard-sidebar,
    ...
    <div class="invoice-container">
        ...
        <div class="totals-section">
            <table class="totals-table">
                ...
                <?php endif; ?>
                <?php 
                // Calculate tax if applicable (you can modify this based on your tax logic)
                $tax_rate = 0; // Set your tax rate here if needed
                $tax_amount = $tax_rate > 0 ? ($subtotal - floatval($booking['discount_amount'])) * ($tax_rate / 100) : 0;
                if ($tax_amount > 0) : 
                ?>
                <tr>
                    <td class="total-label">Tax (<?php echo esc_html($tax_rate); ?>%):</td>
                    <td class="total-amount"><?php echo esc_html( $currency_symbol . number_format_i18n( $tax_amount, 2 ) ); ?></td>
                </tr>
                <?php endif; ?>
                <tr>
...
<?php
// Ensure no further WordPress output
exit;